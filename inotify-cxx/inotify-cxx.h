...
class InotifyEvent {
   ...
   */
    ...
    inline bool IsCreateDir() const {
        return IsType(m_uMask, IN_CREATE | IN_ISDIR);
    }
   ...
   */
    InotifyWatch(const std::string& rPath, int32_t uMask, bool fEnabled = true)
        : m_path(rPath),
          ...
          /**
   * \Brief Create watch on new folders
   *
   * THIS IS NOT IN ORIGINAL inotify-cxx
   * It was added for fbitexpire
   */
          InotifyWatch(const std::string& rPath)
        : m_path(rPath),
...
class Inotify {
   ...
   * \sa SetNonBlock()
   */
    bool WaitForEvents(bool fNoIntr = false) /* throw (InotifyException) */;
